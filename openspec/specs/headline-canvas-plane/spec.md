...
Defines the canvas-texture headline plane for the lighting-interaction block's advance mode. A `CanvasTexture`-backed `PlaneGeometry` composites the block's heading and tagline text into the Three.js scene at `renderOrder = 4`, enabling the text to receive the same pointer-driven vertex displacement as other overlay planes. The DOM text element is visually hidden via a CSS class once the plane is active.
