...
- [ ] 9.3 Manual smoke-test: in advance mode, move pointer rapidly — border vertices on decors and foreground stay still; interior vertices animate
- [ ] 9.4 Manual smoke-test: decor-left and decor-right are visually out of phase (not mirrored)
- [ ] 9.5 Manual smoke-test: vertices near decor anchor barely move on pointer swipe; far vertices move significantly
- [ ] 9.6 Manual smoke-test: adjust `ADVANCE_FG_POINTER_STRENGTH` and observe foreground response differs from decors
- [ ] 9.7 Manual smoke-test: block heading and tagline are visible in headline plane; DOM heading is hidden
- [ ] 9.8 Manual smoke-test: headline plane vertices deform on pointer move and spring back on stop
- [ ] 9.9 Manual smoke-test: browser window resize — headline canvas repaints text at correct new position
