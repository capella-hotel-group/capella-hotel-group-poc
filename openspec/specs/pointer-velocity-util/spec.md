...
Defines the contract for the shared `PointerVelocityTracker` utility (`src/utils/pointer-velocity.ts`). Governs how pointer movement is tracked, how velocity decays over time, and how the tracker's lifecycle is managed.
...
The tracker SHALL implement the following exponential-decay momentum model:
...
`update()` SHALL apply the exponential decay, accumulate `pendingDelta`, and snap to zero when below threshold.
