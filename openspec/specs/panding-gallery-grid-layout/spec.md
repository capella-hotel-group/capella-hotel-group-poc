...
Defines the fixed grid layout contract for the `panding-gallery` block: image count, column distribution, column-width formula, and image aspect ratio.
...
### Requirement: Column width produces a configurable number of visible columns at the viewport width

The `panding-gallery-columns` container SHALL set each column's width using the formula:

`columnWidth = (containerWidth - gap * (COLS - 1)) / VISIBLE_COLS`

where `VISIBLE_COLS` is a configurable constant (default `2.8`) and `COLS = 5`. The column width SHALL be recomputed when the container is resized (via `ResizeObserver`) and applied to the column elements.

#### Scenario: Column width at standard viewport

- **WHEN** the block is rendered at 1440px viewport width
- **THEN** each column is approximately `(1440 - gap * 4) / VISIBLE_COLS` pixels wide
- **AND** only `VISIBLE_COLS` columns are visible without scrolling
