---
name: aem-skill-docs-search
description: Search the aem.live documentation for information on AEM Edge Delivery Services features. Use when you need more information about a feature, want guidance on implementing a feature, and web search isn't returning relevant results. Also check .github/instructions/ for project-specific overrides.
license: Apache-2.0
---
...
## Project-Specific Override: Check Instructions First

Before searching docs, **always check** `.github/instructions/` for project-specific overrides that may supersede the general aem.live guidance:

- `.github/instructions/block-authoring.instructions.md` — block structure, `decorate()` pattern, AEM model JSON, loading phases
- `.github/instructions/coding-style.instructions.md` — TypeScript imports, DOM patterns, CSS tokens, ESLint rules
- `.github/copilot-instructions.md` — key conventions for this TypeScript + Vite project

If the project instructions already answer your question, you don't need to search the docs.
...
- `aem` (filtered as stop word)
- `how to build website` (too broad)
- `edge` (filtered as stop word)
...
```bash
node .github/skills/aem-skill-docs-search/scripts/search.js [--all] <keyword1> [keyword2] [...]
```
...
node .github/skills/aem-skill-docs-search/scripts/search.js block decoration

# Search for metadata with all results
node .github/skills/aem-skill-docs-search/scripts/search.js --all metadata

# Multi-word search
node .github/skills/aem-skill-docs-search/scripts/search.js universal editor blocks
```
...
- `title`: Page title
- `description`: Brief summary (~150 chars) — **use this for quick context**
...
The search results give you an overview. To get detailed information, fetch and read the complete page content.
...
**Best Practice:** Start with the top 2-3 most relevant results, read them fully, then decide if you need more.
...
1. Check `.github/instructions/block-authoring.instructions.md` first — if it covers block decoration for THIS project, use that
2. If you need official docs: `node .github/skills/aem-skill-docs-search/scripts/search.js block decoration`
3. Review top 3 results
4. Fetch the most relevant: `https://www.aem.live/developer/markup-sections-blocks`
5. Read full content and provide answer
...
1. Search: `node .github/skills/aem-skill-docs-search/scripts/search.js metadata`
...
**Good Response:**
"I found information about folder mapping, but **this feature is deprecated**. The deprecation notice says: 'Please contact us if you have a use case for folder mapping...'. The current recommended approach is **Path mapping for AEM authoring** (the top result). Let me read that documentation instead."

## Related Skills

- **aem-skill-building-blocks**: Use when creating new TypeScript blocks from scratch
- **aem-skill-content-modeling**: Use when designing content models for blocks
- **aem-skill-ue-component-model**: Use when configuring Universal Editor component models

## Important Reminders

1. **Always check project instructions first** — `.github/instructions/*.instructions.md` may already answer your question with project-specific guidance that overrides AEM defaults
2. **Always check for deprecation warnings** and alert the user
3. **Fetch and read full pages** — search results are just for finding the right pages
4. **Start with top 2-3 results** before expanding search
5. **The description field is your friend** — it's usually well-written and concise
6. **Don't rely solely on snippets** — they're for context, not comprehensive information
