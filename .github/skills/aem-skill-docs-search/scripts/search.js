...
function extractSnippet(text, keyword) {
  ...
  if (!match) {
    ...
    return extractFirstSentences(cleanedText, 200);
  }
  ...
  let sentenceEnd = cleanedText.length;
  for (let i = matchIndex; i < cleanedText.length; i++) {
    if (cleanedText[i] === '.' || cleanedText[i] === '!' || cleanedText[i] === '?') {
      sentenceEnd = i + 1;
      break;
    }
  ...
  let snippet = cleanedText.substring(sentenceStart, sentenceEnd).trim();

  // If snippet is too short, try to include next sentence
  if (snippet.length < 100 && sentenceEnd < cleanedText.length) {
    let nextSentenceEnd = sentenceEnd;
    for (let i = sentenceEnd + 1; i < cleanedText.length; i++) {
      if (cleanedText[i] === '.' || cleanedText[i] === '!' || cleanedText[i] === '?') {
        nextSentenceEnd = i + 1;
        break;
      }
    }
    snippet = cleanedText.substring(sentenceStart, nextSentenceEnd).trim();
  }
...
function extractFirstSentences(text, maxLength = 200) {
  ...
  for (let i = 0; i < cleanedText.length && i < maxLength + 100; i++) {
    if (cleanedText[i] === '.' || cleanedText[i] === '!' || cleanedText[i] === '?') {
      end = i + 1;
      if (end <= maxLength) {
        lastSentenceEnd = end;
  ...
  if (lastSentenceEnd === 0) {
    ...
    return cleanedText.substring(0, maxLength) + (cleanedText.length > maxLength ? '...' : '');
  }

  return cleanedText.substring(0, lastSentenceEnd).trim();
}
