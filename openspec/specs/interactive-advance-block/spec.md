...
Defines behaviour for an AEM EDS block that renders a full-screen static placeholder on page load and defers loading of a Three.js WebGL scene until the user explicitly interacts. This pattern keeps Three.js (~150–200 kB) out of the initial page load bundle.

> **Note:** originally implemented as the standalone `interactive-advance` block; subsequently merged into `lighting-interaction` (April 2026) which retains identical runtime behaviour while adding heading/tagline overlay and pointer-driven vertex displacement on the scene plane.
...
The block SHALL expose authoring fields via its component model JSON.
